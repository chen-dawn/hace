...
ref_percentages <- out_dataframe_filtered %>% filter(condition == "mix2-day0") %>% ungroup() %>% 
  ...
  select(n_base, exon_num, cds_base, A_ref_count, C_ref_count, G_ref_count, T_ref_count, A_ref_perc, C_ref_perc, G_ref_perc, T_ref_perc, REF_Allele_ref_count, REF_Allele_alt_count) 
...
treated_conditions_only <- out_dataframe_filtered %>% select(sample, condition, n_base, ref_base, A_count, C_count, G_count, T_count, A_perc, C_perc, G_perc, T_perc, Allele_ref_count, Allele_alt_count) %>% 
  # filter(condition != "mix2-day0" & condition != "mix2s-day14" & condition != "mix2t-day14")
  filter(condition %in% c("mix2s-day20", "mix2t-day20"))
...
# write.csv(with_ref_alt_mut_rates, file.path(out_dir, "data_4a_A375_drug_selection_pvalues_day20.csv"))
low_p <- with_ref_alt_mut_rates %>% filter(fisher_res.adj<0.05)
length(unique(low_p$cds_base))

# We are going to plot the RNA data instead because that has the entire range. So here we are only going to plot the p-value data. 
# # Looks like Day 14 data is already not bad. 
g_with_lab <- ggplot(with_ref_alt_mut_rates, aes(log10(control_count_mut_rate), log10(treated_count_mut_rate), color = fisher_res.adj, label = cds_base)) + 
  ...
  scale_colour_gradient2(high = "#E9F6E5",mid = "#5DD0BE",low = "#003674", midpoint = 0.8)
ggsave(file.path(out_dir, "fig4a_p_val_with_label_TMP.pdf"), g_with_lab, units = "cm", width = 20, height = 10, dpi = 300)

# T drug p vals.
g_Tpval <- ggplot(with_ref_alt_mut_rates %>% filter(condition == "mix2t-day20"), aes(log10(control_count_mut_rate), log10(treated_count_mut_rate), color = fisher_res.adj, label = cds_base)) + 
  geom_abline(slope = 1, intercept = 0) + 
  geom_point() + 
  # scale_x_log10() + scale_y_log10() + 
  # facet_wrap(~condition) + 
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.background = element_blank()) + 
  scale_colour_gradient2(high = "#E9F6E5",mid = "#5DD0BE",low = "#003674", midpoint = 0.8)
g_Spval <- ggplot(with_ref_alt_mut_rates %>% filter(condition == "mix2s-day20"), aes(log10(control_count_mut_rate), log10(treated_count_mut_rate), color = fisher_res.adj, label = cds_base)) + 
  geom_abline(slope = 1, intercept = 0) + 
  geom_point() + 
  # scale_x_log10() + scale_y_log10() + 
  # facet_wrap(~condition) + 
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.background = element_blank()) + 
  scale_colour_gradient2(high = "#E9F6E5",mid = "#5DD0BE",low = "#003674", midpoint = 0.8)
ggsave(file.path(out_dir, "fig4a_p_val_TandSdrug.pdf"), arrangeGrob(g_Tpval, g_Spval, nrow = 1), units = "cm", width = 27, height = 10, dpi = 300)

# # Only look at the day14 data and plot. 
# newPalette400_rotate <- c("#F87171","#60A5FA", "#34D399", "#A78BFA", "#FBBF24", "#A3E635", "#E879F9")
# day14s <- with_ref_alt_mut_rates %>% filter(condition =="mix2s-day14") %>% 
...
# 
# 
# day14t <- with_ref_alt_mut_rates %>% filter(condition =="mix2t-day14") %>% 
#   select(n_base,cds_base, A_perc, C_perc, G_perc, T_perc) %>% 
#   pivot_longer(cols = contains("perc"), values_to = "mut_to_ref", names_to = "mut_mode")
# 
...
#   scale_x_continuous(limits = c(0,700), n.breaks = 10)
# 
# ggsave(file.path(out_dir, "fig4a_drug_selection_allele_frequency.pdf"), arrangeGrob(g14t, g14s, nrow = 2), units = "cm", width = 11, height = 10, dpi = 300)
# 
# 
# 
# 
