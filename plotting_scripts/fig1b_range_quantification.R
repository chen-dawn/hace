...
# }
# 
# write.csv(MEK1_pileup, "~/Dropbox (Harvard University)/03Helicase/data/Expt06_Multiple_locations_range_tagment/merged_all_tagment_pileups.csv")

MEK1_pileup <- read_csv("~/Dropbox (Harvard University)/03Helicase/data/Expt06_Multiple_locations_range_tagment/merged_all_tagment_pileups.csv")
MEK1_pileup <- MEK1_pileup[,2:ncol(MEK1_pileup)] %>% 
...
RUNX1 <- MEK1_pileup %>% filter(grepl("RUNX1", sample)) %>% 
  mutate(n_base = n_base - 5153) %>% filter(chr == "RUNX1_8kb_extraction") %>% mutate(loci = "RUNX1")
...
reconcat_all <- rbind(RUNX1, DC203, DC329, DC330, DC331, DC573, DC576, DC577) %>% filter(n_base > -3000 & n_base < 3000)
...
# ggplot(with_mut_background_subtracted %>% filter(ref_base.x == "G"), aes(n_base, GtoA_bg_subtracted, color = Cas9Display)) + ylim(0,2) + 
#   geom_point() + facet_grid(helicaseDisplay+Cas9Display ~ loci) + xlim(-2000,2000) 

# avg_mut_per_base <- with_mut_background_subtracted %>% 
#   pivot_longer(cols = contains("bg_subtracted"), names_to = "editMode") %>% 
#   group_by(chr, n_base, ref_base, editMode, condition, loci, helicaseDisplay, Cas9Display) %>% 
#   summarise(editRate = mean(value), sd = sd(value)) 
# 
# avg_mut_per_base_filtered <- avg_mut_per_base %>% 
#   filter(ref_base %in% c("C", "G"))

# For each loci, we want to quantify the editing rate.
allC_10bp <- with_mut_background_subtracted %>% filter(ref_base == "C") %>% filter(n_base >= -10 & n_base <= 10) %>% 
  filter(!is.na(CtoT_bg_subtracted)) %>%
  group_by(sample,condition, isControl, loci, helicaseDisplay, Cas9Display) %>% 
  summarise(meanCtoT_10bp = mean(CtoT_bg_subtracted))

allC_bigger_than_10bp <- with_mut_background_subtracted %>% filter(ref_base == "C") %>% filter(n_base > 10 & n_base <= 1000) %>% 
  filter(!is.na(CtoT_bg_subtracted)) %>%
  group_by(sample) %>% 
  summarise(meanCtoT_bigger_than_10bp = mean(CtoT_bg_subtracted))

allC_smaller_than_minus10bp <- with_mut_background_subtracted %>% filter(ref_base == "C") %>% filter(n_base < -10 & n_base >=-1000) %>% 
  filter(!is.na(CtoT_bg_subtracted)) %>%
  group_by(sample) %>% 
  summarise(meanCtoT_smaller_than_10bp = mean(CtoT_bg_subtracted))

allG_10bp <- with_mut_background_subtracted %>% filter(ref_base == "G") %>% filter(n_base >= -10 & n_base <= 10) %>% 
  filter(!is.na(GtoA_bg_subtracted)) %>%
  group_by(sample) %>% 
  summarise(meanGtoA_10bp = mean(GtoA_bg_subtracted))

allG_bigger_than_10bp <- with_mut_background_subtracted %>% filter(ref_base == "G") %>% filter(n_base > 10 & n_base <= 1000) %>% 
  filter(!is.na(GtoA_bg_subtracted)) %>%
  group_by(sample) %>% 
  summarise(meanGtoA_bigger_than_10bp = mean(GtoA_bg_subtracted))

allG_smaller_than_minus10bp <- with_mut_background_subtracted %>% filter(ref_base == "G") %>% filter(n_base < -10 & n_base >=-1000) %>% 
  filter(!is.na(GtoA_bg_subtracted)) %>%
  group_by(sample) %>% 
...
avg_across_reps <- merged_stats %>% group_by(condition, isControl, loci, helicaseDisplay, Cas9Display) %>% 
  summarise(meanGtoA_bigger_than_10bp = mean(meanGtoA_bigger_than_10bp), sdGtoA_bigger_than_10bp = sd(meanGtoA_bigger_than_10bp))

ggplot(avg_across_reps, aes(meanGtoA_bigger_than_10bp, condition, fill = Cas9Display)) + geom_bar(stat = "identity") + facet_wrap(~loci) 




##### PLOT NOW. #####
plotting_subset <- avg_across_reps  %>% filter(is.na(Cas9Display) | Cas9Display == "nCas9 D10A")
ggplot(plotting_subset, aes(meanGtoA_bigger_than_10bp, loci, fill = Cas9Display)) + geom_bar(stat = "identity", position = "dodge") + facet_wrap(~helicaseDisplay) 
...
p1a_range <- ggplot(data = plotting_subset, aes(n_base, editRate, color = editMode, ymin=editRate-sd, ymax=editRate+sd)) + 
  ...
  geom_point(size = 1) + 
  ylim(0,2) +
  scale_x_continuous(breaks = round(seq(-1600, 1600, by = 400),1), limits = c(-1600, 1600)) + 
  theme_bw() + 
  scale_color_npg() + 
...
ggsave(file.path(out_dir, "fig1a_range.pdf"), p1a_range, units = "cm", width = 14, height = 7, dpi = 300)
