...
spacer_only <- with_mutation_rate %>% filter(n_base >= -3 & n_base <= 17) %>%
  mutate(modesToT = (`T`/base_sum)* 100) %>% 
  mutate(modesToA = (A/base_sum)* 100) %>% 
  mutate(modesToT = ifelse(ref_base == "T", 0, modesToT)) %>%
  mutate(modesToA = ifelse(ref_base == "A", 0, modesToA))
write.csv(spacer_only, file.path(out_dir, "data_supp1d_spacer_editing.csv"))




spacer_grouped_by_condition <- spacer_only %>% group_by(condition, loci, helicaseDisplay, Cas9Display, n_base, ref_base) %>% 
  summarise(meanModesToT = mean(modesToT), sdModesToT = sd(modesToT), meanModesToA = mean(modesToA), sdModesToA = sd(modesToA))


bars_filtered <- spacer_grouped_by_condition %>% 
  filter(loci == "HEK3") %>% 
  filter(helicaseDisplay == "PcrA M6" | is.na(helicaseDisplay)) %>%
  filter( Cas9Display %in% c("nCas9 D10A")| is.na(Cas9Display))
points_filtered <- spacer_only %>% 
  filter(loci == "HEK3") %>% 
  filter(helicaseDisplay == "PcrA M6"| is.na(helicaseDisplay)) %>%
  filter( Cas9Display %in% c("nCas9 D10A")| is.na(Cas9Display))


p_mean_to_T <- ggplot(bars_filtered , aes(n_base, meanModesToT)) + 
  facet_wrap(~condition, scales='free', ncol = 1) +
  geom_errorbar(aes(ymin=meanModesToT-sdModesToT/2, ymax=meanModesToT+sdModesToT/2), width=.2, position=position_dodge(.9)) +
  geom_bar(stat = "identity", position = "dodge", fill = "#FCBB50", color = "black", size = 0.25, width = 0.7) + 
  # geom_point(data=points_filtered, aes(n_base, modesToT), size=1, show.legend=FALSE) + 
  geom_beeswarm(data = points_filtered, aes(n_base, modesToT),
              color = "black", fill = "white", shape=21, cex = 0.5, size = 1, method = "swarm") + 
  theme_bw()+ 
  scale_fill_manual(values = c("#989898", "#cc0b01")) + 
  xlab("Loci") + 
  ylab("->T Mutation Rate Per Base (%)") + 
  ...
  ylim(0,8) + 
  scale_x_continuous(breaks = -3:17,labels = c("T", "C", "A", "C","G","T","G", "C", "T", "C",
                                               "A","G","T", "C", "T", "G","G","G","C","C","C"))

p_mean_to_A <- ggplot(bars_filtered , aes(n_base, meanModesToA)) + 
  facet_wrap(~condition, scales='free', ncol = 1) +
  geom_errorbar(aes(ymin=meanModesToA-sdModesToA/2, ymax=meanModesToA+sdModesToA/2), width=.2, position=position_dodge(.9)) +
  geom_bar(stat = "identity", position = "dodge", fill = "#FCBB50", color = "black", size = 0.25, width = 0.7) + 
  # geom_point(data=points_filtered, aes(n_base, modesToT), size=1, show.legend=FALSE) + 
  geom_beeswarm(data = points_filtered, aes(n_base, modesToA),
                color = "black", fill = "white", shape=21, cex = 0.5, size = 1, method = "swarm") + 
  theme_bw()+ 
  scale_fill_manual(values = c("#989898", "#cc0b01")) + 
  xlab("Loci") + 
  ylab("->A Mutation Rate Per Base (%)") + 
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
  ...
  scale_x_continuous(breaks = -3:17,labels = c("T", "C", "A", "C","G","T","G", "C", "T", "C",
                                               "A","G","T", "C", "T", "G","G","G","C","C","C"))
g <- arrangeGrob(p_mean_to_T, p_mean_to_A, ncol = 2)
ggsave(file.path(out_dir, "fig1b_supp_spacer_rates.pdf"), g, units = "cm", width = 24, height = 30, dpi = 150)


# 
# ggplot(spacer_grouped_by_condition %>% filter(loci == "HEK3"), aes(n_base, meanModesToT)) + geom_point() + facet_wrap(~sample)
#   
# 
# 
# 
